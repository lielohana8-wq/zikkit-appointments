  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: { xs: 2, sm: 4 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 20, bgcolor: c.chrome, backdropFilter: 'blur(20px)' }}>
        ...
        <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text }}>📊 דוח צוות ושכר</Typography>
        <Box sx={{ width: 90 }} />
      </Box>

      <Box sx={{ maxWidth: 720, mx: 'auto', px: 2, py: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, mb: 2.5 }}>
          <Button onClick={() => shiftMonth(-1)} sx={{ color: c.text2, fontWeight: 600, minWidth: 40 }}>‹</Button>
          <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text, minWidth: 150, textAlign: 'center' }}>{monthLabel(month)}</Typography>
          <Button onClick={() => shiftMonth(1)} sx={{ color: c.text2, fontWeight: 600, minWidth: 40 }}>›</Button>
        </Box>

        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 2.5 }}>
          <Box sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2, textAlign: 'center' }}>
            <Typography sx={{ fontSize: 24, fontWeight: 600, color: c.text }}>₪{totals.revenue.toLocaleString()}</Typography>
            <Typography sx={{ fontSize: 12, color: c.text3 }}>סה&quot;כ הכנסות החודש</Typography>
          </Box>
          <Box sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2, textAlign: 'center' }}>
            <Typography sx={{ fontSize: 24, fontWeight: 600, color: c.text }}>{totals.count}</Typography>
        ...
        {rows.map((r) => (
          <Box key={r.name} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2, mb: 1.5 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              <Typography sx={{ fontSize: 15.5, fontWeight: 600, color: c.text }}>✂️ {r.name}</Typography>
              <Typography sx={{ fontSize: 18, fontWeight: 600, color: c.accent }}>₪{r.revenue.toLocaleString()}</Typography>
            </Box>
