  ...
  return (
    <Box dir="rtl" sx={{ minHeight: '100vh', background: 'radial-gradient(1000px 500px at 50% -10%, #7C3AED33, transparent), linear-gradient(180deg,#0A0710,#141019 45%,#0A0710)', color: '#EDEAF2', fontFamily: 'inherit' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: { xs: 2.5, md: 6 }, py: 2.5, maxWidth: 1100, mx: 'auto' }}>
        <Typography sx={{ fontWeight: 900, fontSize: 20, letterSpacing: '0.12em' }}>ZIKKIT ⚡</Typography>
        <Button onClick={go} sx={{ color: '#EDEAF2', fontWeight: 700, fontSize: 14 }}>כניסה</Button>
      </Box>

      {/* Hero */}
      <Box sx={{ textAlign: 'center', px: 2.5, pt: { xs: 6, md: 10 }, pb: 8, maxWidth: 760, mx: 'auto' }}>
        <Typography sx={{ fontSize: 12.5, fontWeight: 800, color: '#C9A0FF', letterSpacing: '0.2em', mb: 2 }}>למספרות · ציפורניים · קוסמטיקה · כל עסק תורים</Typography>
        <Typography component="h1" sx={{ fontSize: { xs: 38, md: 56 }, fontWeight: 900, lineHeight: 1.15, letterSpacing: '-0.02em', mb: 2.5 }}>
          העסק שלך.<br />
        ...
        <Button onClick={go} sx={{ background: `linear-gradient(135deg, ${ACCENT}, ${PINK})`, color: '#fff', fontWeight: 900, fontSize: 17, px: 5, py: 1.75, borderRadius: 99, boxShadow: `0 18px 44px ${ACCENT}55`, '&:hover': { transform: 'translateY(-2px)', boxShadow: `0 22px 52px ${ACCENT}70` }, transition: 'all .2s' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', gap: { xs: 3, md: 7 }, flexWrap: 'wrap', px: 2.5, pb: 8 }}>
        {[['90%', 'פחות טלפונים ביום'], ['24/7', 'קביעת תורים אונליין'], ['0', 'תורים שנופלים בין הכיסאות']].map(([n, t]) => (
          <Box key={t} sx={{ textAlign: 'center' }}>
            <Typography sx={{ fontSize: 34, fontWeight: 900, background: `linear-gradient(90deg, ${ACCENT}, ${PINK})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{n}</Typography>
      ...
      <Box sx={{ maxWidth: 1000, mx: 'auto', px: 2.5, pb: 9 }}>
        <Typography sx={{ textAlign: 'center', fontSize: { xs: 26, md: 32 }, fontWeight: 900, mb: 4.5 }}>כל מה שעסק תורים צריך. במקום אחד.</Typography>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
          {features.map((f) => (
            <Box key={f.title} sx={{ bgcolor: 'rgba(255,255,255,0.045)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 4, p: 2.75, transition: 'all .25s', '&:hover': { transform: 'translateY(-4px)', borderColor: `${ACCENT}66` } }}>
              ...
              <Typography sx={{ fontSize: 15.5, fontWeight: 800, mb: 0.75 }}>{f.title}</Typography>
      ...
      <Box sx={{ maxWidth: 660, mx: 'auto', px: 2.5, pb: 9, textAlign: 'center' }}>
        <Typography sx={{ fontSize: { xs: 19, md: 23 }, fontWeight: 700, lineHeight: 1.7, color: '#EDEAF2' }}>
          ״פעם הטלפון לא הפסיק לצלצל באמצע תספורות. היום הלקוחות קובעים לבד באפליקציה — ואני רק מסתפר… כלומר מספר.״
        </Typography>
        <Typography sx={{ fontSize: 13.5, color: '#C9A0FF', fontWeight: 800, mt: 2 }}>💈 דניאל · בעל מספרה · לקוח זיקית</Typography>
      </Box>

      {/* How it works */}
      <Box sx={{ maxWidth: 860, mx: 'auto', px: 2.5, pb: 9 }}>
        <Typography sx={{ textAlign: 'center', fontSize: { xs: 26, md: 32 }, fontWeight: 900, mb: 4.5 }}>מ-0 לאפליקציה חיה — ב-5 דקות</Typography>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr 1fr' }, gap: 2 }}>
          {steps.map(([n, t, sub]) => (
            <Box key={n} sx={{ textAlign: 'center', p: 2 }}>
              <Box sx={{ width: 52, height: 52, borderRadius: '50%', mx: 'auto', mb: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 20, background: `linear-gradient(135deg, ${ACCENT}, ${PINK})` }}>{n}</Box>
              <Typography sx={{ fontSize: 16, fontWeight: 800, mb: 0.5 }}>{t}</Typography>
      ...
      <Box sx={{ maxWidth: 460, mx: 'auto', px: 2.5, pb: 9, textAlign: 'center' }}>
        <Box sx={{ bgcolor: 'rgba(255,255,255,0.05)', border: `1.5px solid ${ACCENT}66`, borderRadius: 5, p: 4, boxShadow: `0 24px 60px ${ACCENT}30` }}>
          <Typography sx={{ fontSize: 12.5, fontWeight: 900, color: '#C9A0FF', letterSpacing: '0.15em', mb: 1 }}>⭐ מחיר השקה — מייסדים</Typography>
          <Typography sx={{ fontSize: 52, fontWeight: 900, lineHeight: 1 }}>₪99<Box component="span" sx={{ fontSize: 17, color: '#A79DB5', fontWeight: 700 }}> /חודש</Box></Typography>
          ...
          <Button onClick={go} fullWidth sx={{ mt: 3, background: `linear-gradient(135deg, ${ACCENT}, ${PINK})`, color: '#fff', fontWeight: 900, fontSize: 16, py: 1.5, borderRadius: 99 }}>
            30 יום חינם — מתחילים ←
          </Button>
        </Box>
      </Box>
      ...
      <Box sx={{ textAlign: 'center', px: 2.5, pb: 8 }}>
        <Typography sx={{ fontSize: { xs: 24, md: 30 }, fontWeight: 900, mb: 2 }}>הלקוח הבא שלך כבר מחפש איפה לקבוע.</Typography>
        <Button onClick={go} sx={{ background: `linear-gradient(135deg, ${ACCENT}, ${PINK})`, color: '#fff', fontWeight: 900, fontSize: 16, px: 4.5, py: 1.5, borderRadius: 99 }}>פתחו את האפליקציה של העסק שלכם 🚀</Button>
