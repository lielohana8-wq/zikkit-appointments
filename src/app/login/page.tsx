...
export default function AuthPageWrapper() {
  return (
    <Suspense fallback={<Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: c.canvas }}><CircularProgress sx={{ color: c.accent }} /></Box>}>
      <AuthPage />
    </Suspense>
  );
}
...
function AuthPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas, position: 'relative', overflow: 'hidden', direction: 'rtl' }}>
      ...
      {view === 'landing' && (
        <Box className="zk-fade-up" sx={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', px: 3, py: 4, maxWidth: 680, mx: 'auto' }}>
            <Box sx={{ bgcolor: c.accentDim, color: c.accent, fontWeight: 500, fontSize: 13, borderRadius: 99, px: 2, py: 0.75, mb: 3 }}>
              ✦ מערכת ניהול התורים החכמה לעסקים
            </Box>
            <Typography sx={{ fontSize: { xs: 46, sm: 72 }, fontWeight: 600, color: c.text, letterSpacing: '-0.055em', lineHeight: 0.92, mb: 3 }}>
              היומן שלך מתמלא.<br />
            ...
            <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', justifyContent: 'center' }}>
              <Button onClick={() => goTo('request')} variant="contained" sx={{ py: 1.6, px: 4, borderRadius: 1.5, fontWeight: 500, fontSize: 16 }}>התחל בחינם →</Button>
          ...
          <Box sx={{ maxWidth: 920, mx: 'auto', width: '100%', px: { xs: 3, sm: 6 }, pb: 6 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, 1fr)', sm: 'repeat(4, 1fr)' }, gap: 2 }}>
              {features.map((f) => (
                <Box key={f.title} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5 }}>
                  <Box sx={{ fontSize: 28, mb: 1 }}>{f.icon}</Box>
                  <Typography sx={{ fontSize: 14.5, fontWeight: 500, color: c.text, mb: 0.25 }}>{f.title}</Typography>
      ...
      {view === 'login' && (
        <Box sx={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Box className="zk-fade-up" sx={{ maxWidth: 400, width: '100%' }}>
            ...
            <Box sx={{ bgcolor: c.card, borderRadius: 4, p: { xs: 3, sm: 4.5 }, boxShadow: c.shadowLg, border: `1px solid ${c.border2}` }}>
              <Typography sx={{ fontSize: 24, fontWeight: 600, textAlign: 'center', color: c.text, letterSpacing: '-0.02em' }}>ברוכים השבים</Typography>
              ...
              {error && <Box sx={{ bgcolor: c.hotDim, borderRadius: 4, px: 2, py: 1.25, mb: 2 }}><Typography sx={{ fontSize: 13, color: c.hot, fontWeight: 500 }}>{error}</Typography></Box>}

              <Button onClick={handleSubmit} disabled={loading} fullWidth variant="contained" sx={{ py: 1.5, borderRadius: 1.5, fontWeight: 500, fontSize: 15.5 }}>
            ...
            <Typography sx={{ fontSize: 14, color: c.text2, textAlign: 'center', mt: 3 }}>
              עדיין לא בפיילוט?{' '}
              <Box component="span" onClick={() => goTo('request')} sx={{ color: c.accent, fontWeight: 500, cursor: 'pointer' }}>בקשו גישה</Box>
            </Typography>
          </Box>
        </Box>
      )}
      ...
      {view === 'request' && (
        <Box sx={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Box className="zk-fade-up" sx={{ maxWidth: 420, width: '100%' }}>
            ...
            <Box sx={{ bgcolor: c.card, borderRadius: 4, p: { xs: 3, sm: 4.5 }, boxShadow: c.shadowLg, border: `1px solid ${c.border2}` }}>
              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.75, bgcolor: c.accentDim, color: c.accent, borderRadius: 99, px: 1.5, py: 0.5, fontSize: 12.5, fontWeight: 500, mb: 1.5 }}>🎁 פיילוט · חינם</Box>
              <Typography sx={{ fontSize: 24, fontWeight: 600, color: c.text, letterSpacing: '-0.02em' }}>בקשה להצטרף לפיילוט</Typography>
              ...
              {reqError && <Box sx={{ bgcolor: c.hotDim, borderRadius: 4, px: 2, py: 1.25, mb: 2 }}><Typography sx={{ fontSize: 13, color: c.hot, fontWeight: 500 }}>{reqError}</Typography></Box>}

              <Button onClick={submitRequest} disabled={loading || !reqName.trim() || !reqPhone.trim()} fullWidth variant="contained" sx={{ py: 1.5, borderRadius: 1.5, fontWeight: 500, fontSize: 15.5 }}>
            ...
            <Typography sx={{ fontSize: 14, color: c.text2, textAlign: 'center', mt: 3 }}>
              יש לכם קוד הזמנה?{' '}
              <Box component="span" onClick={() => goTo('signup')} sx={{ color: c.accent, fontWeight: 500, cursor: 'pointer' }}>הירשמו כאן</Box>
            </Typography>
            <Typography sx={{ fontSize: 14, color: c.text2, textAlign: 'center', mt: 1 }}>
              כבר יש לכם חשבון?{' '}
              <Box component="span" onClick={() => goTo('login')} sx={{ color: c.accent, fontWeight: 500, cursor: 'pointer' }}>התחברו</Box>
            </Typography>
          </Box>
        </Box>
      )}
      ...
      {view === 'signup' && (
        <Box sx={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Box className="zk-fade-up" sx={{ maxWidth: 420, width: '100%' }}>
            ...
            <Box sx={{ bgcolor: c.card, borderRadius: 4, p: { xs: 3, sm: 4.5 }, boxShadow: c.shadowLg, border: `1px solid ${c.border2}` }}>
              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.75, bgcolor: c.accentDim, color: c.accent, borderRadius: 99, px: 1.5, py: 0.5, fontSize: 12.5, fontWeight: 500, mb: 1.5 }}>🎉 אושרת לפיילוט</Box>
              <Typography sx={{ fontSize: 24, fontWeight: 600, color: c.text, letterSpacing: '-0.02em' }}>יצירת החשבון שלך</Typography>
              ...
              <TextField fullWidth placeholder="קוד הזמנה (ZK-XXXX)" value={inviteCode} onChange={(e) => setInviteCode(e.target.value.toUpperCase())} sx={{ mb: 1.75, '& input': { fontFamily: 'monospace', letterSpacing: '0.1em', fontWeight: 500 } }} />
              <TextField fullWidth placeholder="שם העסק" value={bizName} onChange={(e) => setBizName(e.target.value)} sx={{ mb: 1.75 }} />
              ...
              {signupError && <Box sx={{ bgcolor: c.hotDim, borderRadius: 4, px: 2, py: 1.25, mb: 2 }}><Typography sx={{ fontSize: 13, color: c.hot, fontWeight: 500 }}>{signupError}</Typography></Box>}

              <Button onClick={verifyAndSignup} disabled={loading || !inviteCode.trim() || !email.trim() || !password.trim()} fullWidth variant="contained" sx={{ py: 1.5, borderRadius: 1.5, fontWeight: 500, fontSize: 15.5 }}>
      ...
      {view === 'sent' && (
        <Box sx={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
          <Box className="zk-fade-up" sx={{ maxWidth: 400, width: '100%', textAlign: 'center' }}>
            ...
            <Typography sx={{ fontSize: 28, fontWeight: 600, color: c.text, letterSpacing: '-0.03em', mb: 1.5 }}>קיבלנו את הבקשה!</Typography>
            ...
            <Button onClick={() => goTo('landing')} variant="outlined" sx={{ borderRadius: 1.5, fontWeight: 500, px: 4, py: 1.25 }}>חזרה לדף הבית</Button>
          </Box>
        </Box>
      )}
    </Box>
