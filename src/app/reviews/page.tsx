  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: { xs: 2.5, sm: 4 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: c.chrome, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text }}>ביקורות</Typography>
        <Button onClick={() => setOpen(true)} variant="contained" sx={{ borderRadius: 99, fontWeight: 500, px: 2.5 }}>+ ביקורת</Button>
      </Box>

      <Box className="zk-page" sx={{ maxWidth: 680, mx: 'auto', px: { xs: 2.5, sm: 4 }, py: 3 }}>
        {/* Stats hero */}
        <Box sx={{ bgcolor: c.accent, borderRadius: 4, p: { xs: 3, sm: 4 }, color: '#fff', mb: 3, boxShadow: c.shadowLg, textAlign: 'center' }}>
          <Typography sx={{ fontSize: 52, fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 1 }}>{stats.avg || '—'}</Typography>
            ...
            {[5, 4, 3, 2, 1].map((star) => {
              ...
              return (
                <Box key={star} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                  ...
                  <Box sx={{ flex: 1, height: 8, bgcolor: c.fill, borderRadius: 99, overflow: 'hidden' }}>
        ...
        {reviews.length === 0 ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            {reviews.map((r) => (
              <Box key={r.id} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1 }}>
                  <Box sx={{ width: 40, height: 40, borderRadius: '50%', bgcolor: c.accent, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: 16 }}>{r.customerName?.[0] || '?'}</Box>
                  <Box sx={{ flex: 1 }}>
                    <Typography sx={{ fontSize: 14.5, fontWeight: 500, color: c.text }}>{r.customerName}</Typography>
      ...
      <Dialog scroll="body" open={open} onClose={() => setOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 3.5, maxWidth: 400, width: '100%' } }}>
        <Typography sx={{ fontSize: 21, fontWeight: 600, mb: 2.5, color: c.text }}>הוסף ביקורת</Typography>
        <TextField fullWidth label="שם הלקוח" value={draft.customerName} onChange={(e) => setDraft((p) => ({ ...p, customerName: e.target.value }))} sx={{ mb: 2 }} />
        <Typography sx={{ fontSize: 13, fontWeight: 500, color: c.text2, mb: 1 }}>דירוג</Typography>
        ...
        <Button onClick={save} variant="contained" fullWidth disabled={!draft.customerName || !draft.text || saving} sx={{ borderRadius: 1.5, fontWeight: 500, py: 1.5 }}>
