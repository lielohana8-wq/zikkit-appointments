  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: { xs: 2.5, sm: 4 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: c.chrome, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text }}>📈 מרכז צמיחה</Typography>
        <Box sx={{ width: 60 }} />
      </Box>
      ...
      <Box className="zk-page" sx={{ maxWidth: 760, mx: 'auto', px: { xs: 2, sm: 4 }, py: 3, display: 'flex', flexDirection: 'column', gap: 3.5 }}>
        ...
        <Box sx={{ background: `linear-gradient(135deg, ${c.accent}, #9333EA)`, borderRadius: 4, p: { xs: 2.5, sm: 3.5 }, color: '#fff', boxShadow: '0 16px 48px rgba(124,58,237,0.35)' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
            <Box>
              <Typography sx={{ fontSize: 14, fontWeight: 500, opacity: 0.9 }}>ציון הצמיחה שלך</Typography>
              <Typography sx={{ fontSize: 56, fontWeight: 600, lineHeight: 1, letterSpacing: '-0.03em' }}>{score.total}<span style={{ fontSize: 26, opacity: 0.75 }}>/100</span></Typography>
            </Box>
            ...
            {score.subs.map((s) => (
              <Box key={s.key}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.4 }}>
                  <Typography sx={{ fontSize: 13, fontWeight: 500 }}>{s.icon} {s.label}</Typography>
                  <Typography sx={{ fontSize: 13, fontWeight: 600 }}>{s.score}</Typography>
                </Box>
        ...
        <Box sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: { xs: 2.5, sm: 3 } }}>
          <Typography sx={{ fontSize: 13, fontWeight: 600, color: c.accent, mb: 0.75 }}>🧠 מנהל השיווק שלך</Typography>
          <Typography sx={{ fontSize: 19, fontWeight: 600, color: c.text, mb: 1.5 }}>בוקר טוב! השבוע כדאי לך:</Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {brief.tasks.map((t, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 1.25, alignItems: 'center' }}>
                <Box sx={{ width: 26, height: 26, borderRadius: '50%', bgcolor: c.accentDim, color: c.accent, fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</Box>
                <Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.text }}>{t}</Typography>
              </Box>
            ))}
          </Box>
          {brief.potential > 0 && (
            <Typography sx={{ fontSize: 14, fontWeight: 600, color: c.green, mt: 1.75 }}>💰 שווי משוער: ‎+₪{brief.potential.toLocaleString()}</Typography>
          )}
          <Button onClick={() => openGenerator('החזרת לקוחות', 'inactive')} variant="contained" sx={{ mt: 2, borderRadius: 4, fontWeight: 600 }}>⚡ בצע הכל — התחל מקמפיין</Button>
        </Box>

        {/* ===== 2. AI OPPORTUNITIES ===== */}
        <Box>
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>💡 הזדמנויות שה-AI זיהה</Typography>
          {ops.length === 0 ? (
            <Box sx={{ bgcolor: c.card, border: `1px dashed ${c.border2}`, borderRadius: 4, p: 3, textAlign: 'center' }}>
              <Typography sx={{ fontSize: 14, color: c.text3 }}>ככל שיצטברו תורים ולקוחות — יופיעו כאן הזדמנויות מדויקות 🌱</Typography>
            </Box>
          ) : (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              {ops.map((o) => (
                <Box key={o.id} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.25 }}>
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                    <Box sx={{ width: 44, height: 44, borderRadius: 4, bgcolor: c.accentDim, fontSize: 21, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{o.icon}</Box>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography sx={{ fontSize: 15, fontWeight: 600, color: c.text }}>{o.title}</Typography>
                      <Typography sx={{ fontSize: 13, color: c.text3, mt: 0.25 }}>{o.body}</Typography>
                      {o.potential ? <Typography sx={{ fontSize: 14, fontWeight: 600, color: c.green, mt: 0.75 }}>פוטנציאל: ₪{o.potential.toLocaleString()}</Typography> : null}
                    </Box>
                  </Box>
                  <Button onClick={() => openGenerator(o.id === 'reviews' ? 'בקשת ביקורות' : o.id === 'weekday' ? 'מילוי יומן השבוע' : 'החזרת לקוחות', o.segmentId === 'loyal' ? 'loyal' : o.segmentId || 'inactive')} size="small" variant="outlined" sx={{ mt: 1.5, borderRadius: 4, fontWeight: 500 }}>{o.action} ←</Button>
                </Box>
              ))}
            </Box>
          )}
        ...
        {/* ===== 4. SMART SEGMENTS ===== */}
        <Box>
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>👥 פילוחי לקוחות חכמים</Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {segments.filter((s) => s.customers.length > 0).map((s) => (
              <Box key={s.id} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, overflow: 'hidden' }}>
                <Box onClick={() => setOpenSeg(openSeg === s.id ? null : s.id)} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1.5, p: 2 }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.text }}>{s.label} · {s.customers.length}</Typography>
                    <Typography sx={{ fontSize: 12, color: c.text3 }}>{s.desc}{s.revenue > 0 ? ` · הכניסו ₪${s.revenue.toLocaleString()}` : ''}</Typography>
                  </Box>
                  <Button onClick={(e) => { e.stopPropagation(); openGenerator(s.id === 'loyal' ? 'בקשת ביקורות' : s.id === 'inactive' || s.id === 'at-risk' ? 'החזרת לקוחות' : 'מילוי יומן השבוע', s.id); }} size="small" variant="outlined" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12, whiteSpace: 'nowrap' }}>שלח קמפיין</Button>
                  <Box sx={{ color: c.text3, fontWeight: 500 }}>{openSeg === s.id ? '▾' : '‹'}</Box>
                </Box>
                  ...
                  <Box sx={{ borderTop: `1px solid ${c.border}`, px: 2, py: 1.5, display: 'flex', flexDirection: 'column', gap: 0.75, maxHeight: 260, overflowY: 'auto' }}>
                    {s.customers.slice(0, 25).map((cu) => (
                      <Box key={cu.id} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Button href={waLink(cu.phone, `היי ${cu.name?.split(' ')[0] || ''} 💜`)} target="_blank" size="small" sx={{ minWidth: 'auto', fontSize: 12, fontWeight: 500, color: '#25D366' }}>💬</Button>
                      </Box>
                    ))}
                  </Box>
                </Collapse>
        ...
        <Box>
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>🔮 תחזיות הכנסה</Typography>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
            {preds.map((p, i) => (
              <Box key={i} sx={{ background: `linear-gradient(160deg, ${c.surface1}, ${c.surface2})`, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.25 }}>
                <Typography sx={{ fontSize: 13.5, fontWeight: 500, color: c.text2 }}>{p.icon} {p.title}</Typography>
                {p.amount > 0 && <Typography sx={{ fontSize: 30, fontWeight: 600, color: c.green, letterSpacing: '-0.02em', my: 0.5 }}>‎+₪{p.amount.toLocaleString()}<span style={{ fontSize: 14, color: c.text3, fontWeight: 600 }}> {p.period}</span></Typography>}
        ...
        <Box>
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>🗓️ לוח השיווק השבועי</Typography>
          <Box sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.25 }}>
            {calendar.map((d, i) => (
              <Box key={d.day} sx={{ display: 'flex', gap: 1.75, position: 'relative', pb: i < calendar.length - 1 ? 2.25 : 0 }}>
                ...
                <Box>
                  <Typography sx={{ fontSize: 12, fontWeight: 600, color: c.accent }}>יום {d.day}</Typography>
                  <Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.text }}>{d.title}</Typography>
        ...
        {/* ===== 9. BENCHMARKS ===== */}
        <Box>
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>📊 איך אתה מול עסקים כמוך?</Typography>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {benches.map((b, i) => (
              <Box key={i} sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', bgcolor: c.card, border: `1px solid ${b.level === 'good' ? c.green + '44' : '#F59E0B44'}`, borderRadius: 4, p: 2 }}>
                <Box sx={{ fontSize: 20 }}>{b.icon}</Box>
                <Box>
                  <Typography sx={{ fontSize: 14, fontWeight: 600, color: c.text }}>{b.title}</Typography>
        ...
        <Box onClick={() => router.push('/ai-studio')} className="zk-card" sx={{ cursor: 'pointer', background: `linear-gradient(135deg, ${c.accentDim}, transparent)`, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5, display: 'flex', alignItems: 'center', gap: 2 }}>
          <Box sx={{ fontSize: 30 }}>🎨</Box>
          <Box sx={{ flex: 1 }}>
            <Typography sx={{ fontSize: 15.5, fontWeight: 600, color: c.text }}>סטודיו תוכן AI</Typography>
            <Typography sx={{ fontSize: 13, color: c.text3 }}>תמונה → פוסט לאינסטגרם · בניית דף נחיתה אוטומטי</Typography>
          </Box>
          <Box sx={{ color: c.accent, fontSize: 20, fontWeight: 500 }}>‹</Box>
        </Box>

        {/* Recent campaigns */}
        {campaigns.length > 0 && (
          <Box>
            <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text, mb: 1.5 }}>📨 קמפיינים אחרונים</Typography>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {campaigns.slice(0, 5).map((cp, i) => (
                <Box key={i} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 1.75 }}>
      ...
      <Dialog open={genOpen} onClose={() => setGenOpen(false)} fullWidth PaperProps={{ sx: { bgcolor: c.card, borderRadius: 4, maxWidth: 480, m: 2 } }}>
        <Box sx={{ p: 3 }}>
          <Typography sx={{ fontSize: 19, fontWeight: 600, color: c.text, mb: 0.5 }}>✨ מחולל קמפיינים</Typography>
          ...
          <Button onClick={runGenerator} fullWidth variant="contained" sx={{ borderRadius: 4, fontWeight: 600, py: 1.25 }}>🎯 צור קמפיין</Button>

          {genOut && (
            <Box sx={{ mt: 2.5, bgcolor: c.fill, borderRadius: 4, p: 2 }}>
              <Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.accent, mb: 1 }}>{genOut.headline}</Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1, mt: 2, flexWrap: 'wrap' }}>
                <Button onClick={() => copyMsg(genOut.message)} variant="contained" size="small" sx={{ borderRadius: 4, fontWeight: 500 }}>📋 העתק לכולם</Button>
                {genSegment?.customers[0] && (
                  <Button href={waLink(genSegment.customers[0].phone, genOut.message.replace(/\{name\}/g, genSegment.customers[0].name?.split(' ')[0] || ''))} target="_blank" onClick={() => saveCampaign(genOut.message)} variant="outlined" size="small" sx={{ borderRadius: 4, fontWeight: 500, color: '#25D366', borderColor: '#25D36655' }}>💬 שלח לראשון בוואטסאפ</Button>
                )}
              </Box>
