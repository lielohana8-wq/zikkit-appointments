  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: { xs: 2.5, sm: 4 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: c.chrome, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text }}>מבצעים ונאמנות</Typography>
        <Button onClick={() => setOpen(true)} variant="contained" sx={{ borderRadius: 99, fontWeight: 500, px: 2.5 }}>+ קופון</Button>
      </Box>

      <Box className="zk-page" sx={{ maxWidth: 680, mx: 'auto', px: { xs: 2.5, sm: 4 }, py: 3 }}>
        {/* Loyalty program */}
        <Typography sx={{ fontSize: 13, fontWeight: 500, color: c.text3, mb: 1.5, textTransform: 'uppercase', letterSpacing: '0.06em' }}>תוכנית נאמנות</Typography>
        <Box sx={{ bgcolor: loyalty.enabled ? `linear-gradient(135deg, ${c.accent}, ${c.accentDeep})` : c.surface1, background: loyalty.enabled ? `linear-gradient(135deg, ${c.accent}, ${c.accentDeep})` : c.surface1, border: `1px solid ${loyalty.enabled ? 'transparent' : c.border}`, borderRadius: 4, p: 3, mb: 4, color: loyalty.enabled ? '#fff' : c.text }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: loyalty.enabled ? 2.5 : 0 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Box>
                <Typography sx={{ fontSize: 16, fontWeight: 600 }}>כרטיסיית נאמנות</Typography>
          ...
          {loyalty.enabled && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 1.5 }}>
                <TextField type="number" label="ביקורים להטבה" value={loyalty.visitsForReward} onChange={(e) => saveLoyaltySettings({ ...loyalty, visitsForReward: Number(e.target.value) })} size="small" sx={{ flex: 1, bgcolor: '#fff', borderRadius: 4 }} />
                <TextField label="ההטבה" value={loyalty.rewardDescription} onChange={(e) => saveLoyaltySettings({ ...loyalty, rewardDescription: e.target.value })} size="small" sx={{ flex: 2, bgcolor: '#fff', borderRadius: 4 }} />
              </Box>
            </Box>
          )}
        </Box>
        ...
        {/* Coupons */}
        <Typography sx={{ fontSize: 13, fontWeight: 500, color: c.text3, mb: 1.5, textTransform: 'uppercase', letterSpacing: '0.06em' }}>קופונים</Typography>
        {promos.length === 0 ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            {promos.map((p) => {
              ...
              return (
                <Box key={p.id} sx={{ bgcolor: c.card, border: `1px dashed ${p.active && !expired ? c.accent : c.border2}`, borderRadius: 4, p: 2.5, opacity: p.active && !expired ? 1 : 0.6 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ bgcolor: c.accentDim, color: c.accent, borderRadius: 4, px: 2, py: 1.25, fontWeight: 600, fontSize: 16, letterSpacing: '0.05em', fontFamily: 'monospace' }}>{p.code}</Box>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography sx={{ fontSize: 18, fontWeight: 600, color: c.text }}>{p.discountType === 'percent' ? `${p.discountValue}% הנחה` : `₪${p.discountValue} הנחה`}</Typography>
      ...
      <Dialog scroll="body" open={open} onClose={() => setOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 3.5, maxWidth: 400, width: '100%' } }}>
        <Typography sx={{ fontSize: 21, fontWeight: 600, mb: 2.5, color: c.text }}>קופון חדש</Typography>
        ...
        <Button onClick={save} variant="contained" fullWidth disabled={!draft.code || saving} sx={{ borderRadius: 1.5, fontWeight: 500, py: 1.5 }}>
