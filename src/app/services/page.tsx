  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: c.chrome, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text }}>מחירון ושירותים</Typography>
        {!isStaff && <Button onClick={openNew} variant="contained" sx={{ borderRadius: 99, fontWeight: 500 }}>+ שירות</Button>}
      </Box>
      ...
      <Box className="zk-page" sx={{ maxWidth: 680, mx: 'auto', px: { xs: 2.5, sm: 4 }, py: 3 }}>
        ...
        {services.length === 0 ? (
          <Box sx={{ textAlign: 'center', py: 6 }}>
            ...
            <Typography sx={{ color: c.text2, mb: 0.5, fontWeight: 500 }}>עדיין אין שירותים</Typography>
            <Typography sx={{ color: c.text3, fontSize: 13.5, mb: 3 }}>הוסף ידנית או תן ל-AI להציע לפי סוג העסק</Typography>
            <Box sx={{ display: 'flex', gap: 1.5, justifyContent: 'center' }}>
              <Button onClick={openNew} variant="contained" sx={{ borderRadius: 1.5, fontWeight: 500 }}>+ הוסף ידני</Button>
              <Button onClick={suggestWithAI} disabled={aiLoading} variant="outlined" sx={{ borderRadius: 1.5, fontWeight: 500 }}>
          ...
          <>
            <Button onClick={suggestWithAI} disabled={aiLoading} fullWidth variant="outlined" sx={{ borderRadius: 1.5, fontWeight: 500, mb: 3, py: 1.25, borderStyle: 'dashed' }}>
              {aiLoading ? <><CircularProgress size={16} sx={{ color: c.accent, mr: 1 }} />מזהה...</> : '✨ הוסף עוד שירותים עם AI'}
            </Button>

            {categories.map((cat) => (
              <Box key={cat} sx={{ mb: 3.5 }}>
                <Typography sx={{ fontSize: 12.5, fontWeight: 500, color: c.text3, mb: 1.5, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{cat}</Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
                  {visibleServices.filter((s) => (s.category || 'כללי') === cat).map((s) => (
                    <Box key={s.id} sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.25, display: 'flex', alignItems: 'center', gap: 2, transition: 'all 0.2s', '&:hover': { boxShadow: c.shadowMd } }}>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Typography sx={{ fontSize: 15.5, fontWeight: 500, color: c.text }}>{s.name}</Typography>
                        {s.description && <Typography sx={{ fontSize: 12.5, color: c.text3, mt: 0.25 }}>{s.description}</Typography>}
                        <Box sx={{ display: 'flex', gap: 0.75, mt: 0.75 }}>
                          <Box sx={{ fontSize: 10.5, fontWeight: 600, bgcolor: c.fill, color: c.text2, borderRadius: 99, px: 1, py: 0.2 }}>🕐 {s.duration} דק'</Box>
                      ...
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography sx={{ fontSize: 21, fontWeight: 600, color: c.accent, letterSpacing: '-0.02em', lineHeight: 1 }}>{s.priceFrom ? <span style={{ fontSize: 12, fontWeight: 500 }}>{'החל מ־'}</span> : null}₪{isStaff && myPrices[s.name] !== undefined ? myPrices[s.name] : s.price}</Typography>
                      </Box>
      ...
      <Dialog scroll="body" open={open} onClose={() => setOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 3.5, maxWidth: 440, width: '100%' } }}>
        <Typography sx={{ fontSize: 21, fontWeight: 600, mb: 2.5, color: c.text }}>{editId ? 'עריכת שירות' : 'שירות חדש'}</Typography>
        ...
        <Box onClick={() => setDraft((p) => ({ ...p, priceFrom: !p.priceFrom }))} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1.25, mb: 2, mt: -0.5 }}>
          <Box sx={{ width: 20, height: 20, borderRadius: 1, border: `2px solid ${draft.priceFrom ? c.accent : c.border2}`, bgcolor: draft.priceFrom ? c.accent : 'transparent', color: '#fff', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>{draft.priceFrom ? '✓' : ''}</Box>
        ...
        <Button onClick={save} variant="contained" fullWidth disabled={!draft.name || saving} sx={{ borderRadius: 1.5, fontWeight: 500, py: 1.5 }}>
