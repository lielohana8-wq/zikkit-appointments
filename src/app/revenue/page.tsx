...
export default function RevenuePage() {
  ...
  const chip = (active: boolean) => ({ cursor: 'pointer', px: 1.5, py: 0.6, borderRadius: 99, fontSize: 12.5, fontWeight: 500, border: `1.5px solid ${active ? c.accent : c.border2}`, color: active ? c.accent : c.text3, bgcolor: active ? c.accentDim : 'transparent', transition: 'all 0.15s' });
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: c.canvas }}>
      ...
      <Box sx={{ borderBottom: `1px solid ${c.border}`, py: 1.75, px: { xs: 2, sm: 4 }, display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: c.chrome, backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 20 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography sx={{ fontSize: 16, fontWeight: 600, color: c.text }}>מנוע ההכנסות</Typography>
        </Box>
        <Button onClick={load} sx={{ color: c.text3, fontWeight: 600, minWidth: 'auto' }}>↻</Button>
      </Box>

      {/* Tabs */}
      <Box sx={{ borderBottom: `1px solid ${c.border2}`, px: { xs: 1, sm: 4 }, display: 'flex', gap: 0.5, bgcolor: c.card, position: 'sticky', top: 57, zIndex: 19 }}>
        {tabs.map(([t, icon, label]) => (
          <Box key={t} onClick={() => setTab(t)} sx={{ cursor: 'pointer', px: 2, py: 1.5, fontSize: 14, fontWeight: 500, color: tab === t ? c.accent : c.text3, borderBottom: `2px solid ${tab === t ? c.accent : 'transparent'}`, display: 'flex', alignItems: 'center', gap: 0.75 }}>
      ...
      <Box className="zk-page" sx={{ maxWidth: 800, mx: 'auto', px: { xs: 2, sm: 4 }, py: 3 }}>
        ...
        {!hoursConfigured && (
          <Box onClick={() => router.push('/hours')} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1.5, bgcolor: '#F59E0B18', border: '1px solid #F59E0B55', borderRadius: 4, px: 2, py: 1.5, mb: 2.5 }}>
            <Box sx={{ fontSize: 18 }}>🕐</Box>
            <Typography sx={{ fontSize: 13.5, fontWeight: 500, color: c.text, flex: 1 }}>שעות הפעילות לא הוגדרו — החורים מחושבים לפי ברירת מחדל. הגדר אותן לדיוק מלא ←</Typography>
          </Box>
        )}
        ...
        {tab === 'radar' && (
          <>
            <Box sx={{ background: `linear-gradient(135deg, ${c.accent}, ${c.accentDeep})`, borderRadius: 4, p: { xs: 3, sm: 4 }, mb: 3, color: '#fff', position: 'relative', overflow: 'hidden' }}>
              ...
              <Typography sx={{ fontSize: 13, fontWeight: 500, opacity: 0.85, position: 'relative' }}>פוטנציאל הכנסה ריאלי</Typography>
              <Typography sx={{ fontSize: { xs: 44, sm: 56 }, fontWeight: 600, letterSpacing: '-0.04em', lineHeight: 1, position: 'relative', my: 0.5 }}>₪{(radar.potentialFromGaps + radar.churningValue).toLocaleString()}</Typography>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5, mb: 3 }}>
              <Box className="zk-card" sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5 }}>
                <Typography sx={{ fontSize: 30, fontWeight: 600, color: c.hot, letterSpacing: '-0.03em' }}>₪{radar.lostToNoShows.toLocaleString()}</Typography>
              ...
              <Box className="zk-card" sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5 }}>
                <Typography sx={{ fontSize: 30, fontWeight: 600, color: c.accent, letterSpacing: '-0.03em' }}>{radar.emptySlotHours} שעות</Typography>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {gaps.length > 0 && (
                <Box onClick={() => setTab('gaps')} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2, bgcolor: c.accentDim, borderRadius: 4, p: 2 }}>
                  <Box sx={{ fontSize: 24 }}>🔮</Box>
                  <Box sx={{ flex: 1 }}><Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.text }}>מלא {gaps.length} חורים ביומן</Typography><Typography sx={{ fontSize: 12.5, color: c.text3 }}>הודעה מוכנה לכל חור — בוואטסאפ</Typography></Box>
                  <Box sx={{ color: c.accent, fontWeight: 600 }}>‹</Box>
                </Box>
              )}
              {churning.length > 0 && (
                <Box onClick={() => setTab('winback')} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2, bgcolor: c.accentDim, borderRadius: 4, p: 2 }}>
                  <Box sx={{ fontSize: 24 }}>🎯</Box>
                  <Box sx={{ flex: 1 }}><Typography sx={{ fontSize: 14.5, fontWeight: 600, color: c.text }}>החזר {churning.length} לקוחות שנעלמו</Typography><Typography sx={{ fontSize: 12.5, color: c.text3 }}>שווי משוער ₪{radar.churningValue.toLocaleString()}</Typography></Box>
                  <Box sx={{ color: c.accent, fontWeight: 600 }}>‹</Box>
                </Box>
              )}
            </Box>
        ...
        {tab === 'gaps' && (
          <>
            {/* Controls */}
            <Box sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2, mb: 2.5 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', mb: 1.5 }}>
                <Typography sx={{ fontSize: 12.5, fontWeight: 500, color: c.text3 }}>ימים קדימה:</Typography>
                {[3, 5, 7].map((d) => <Box key={d} onClick={() => setDaysAhead(d)} sx={chip(daysAhead === d)}>{d}</Box>)}
                <Box sx={{ width: 10 }} />
                <Typography sx={{ fontSize: 12.5, fontWeight: 500, color: c.text3 }}>חור מינימלי:</Typography>
            ...
            {gaps.length === 0 ? (
              <Box sx={{ textAlign: 'center', py: 8 }}>
                ...
                <Typography sx={{ fontWeight: 500, color: c.text }}>היומן שלך מלא!</Typography>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {gaps.map((g, i) => {
                  ...
                  return (
                    <Box key={k} className="zk-card" sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.5 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Box sx={{ textAlign: 'center', bgcolor: c.accentDim, borderRadius: 4, px: 1.75, py: 0.75, minWidth: 68 }}>
                          <Typography sx={{ fontSize: 11, fontWeight: 500, color: c.accent }}>{dayLabel(g.date)}</Typography>
                          <Typography sx={{ fontSize: 15, fontWeight: 600, color: c.accent, letterSpacing: '-0.02em' }}>{g.start}</Typography>
                        </Box>
                        <Box sx={{ flex: 1 }}>
                          <Typography sx={{ fontSize: 15, fontWeight: 600, color: c.text }}>{g.start}–{g.end}</Typography>
                          <Typography sx={{ fontSize: 12.5, color: c.text3 }}>{g.minutes} דקות פנויות · שווי ≈ ₪{Math.round((g.minutes / 45) * ticket)}</Typography>
                        </Box>
                        <Button onClick={() => { const next = { ...dismissed, [k]: true }; setDismissed(next); persistRevenue({ dismissedGaps: next }); }} size="small" sx={{ minWidth: 'auto', fontSize: 12, fontWeight: 500, color: c.text3 }}>✓ טופל</Button>
                      </Box>
                      ...
                      <Box sx={{ borderTop: `1px solid ${c.border2}`, pt: 1.5, mt: 1.75 }}>
                            ...
                            {matches.map((m) => (
                              <Box key={m.customer.id} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                  <Typography sx={{ fontSize: 13.5, fontWeight: 500, color: c.text }}>{m.customer.name}</Typography>
                                  <Typography sx={{ fontSize: 11.5, color: m.tier === 1 ? c.green : c.text3 }}>{m.tier === 1 ? '🎯 ' : ''}{m.reason}</Typography>
                                </Box>
                                <Button onClick={() => openMsg(m.customer.phone, m.customer.name, gapMessage(g, m.customer.name))} size="small" variant="contained" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12, bgcolor: '#25D366', '&:hover': { bgcolor: '#1EA952' } }}>💬 הצע</Button>
                              </Box>
                            ))}
                          </Box>
                        ) : (
                        ...
                        <Box sx={{ display: 'flex', gap: 1, mt: 1.25, flexWrap: 'wrap' }}>
                          <Button onClick={() => { setPickerFor(pickerFor === k ? null : k); setPickQuery(''); }} size="small" variant="outlined" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12 }}>👤 בחר לקוח אחר</Button>
                          <Button onClick={() => { navigator.clipboard?.writeText(gapMessage(g)); }} size="small" variant="outlined" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12 }}>📋 העתק לסטטוס/קבוצה</Button>
                        </Box>
                        <Collapse in={pickerFor === k}>
                          <Box sx={{ mt: 1.5, bgcolor: c.fill, borderRadius: 4, p: 1.5 }}>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, maxHeight: 220, overflowY: 'auto' }}>
                              {pickList.map((cu) => (
                                <Box key={cu.id} onClick={() => { setPickerFor(null); openMsg(cu.phone, cu.name, gapMessage(g, cu.name)); }} sx={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1, px: 1.25, py: 0.9, borderRadius: 1.5, '&:hover': { bgcolor: c.fill } }}>
        ...
        {tab === 'winback' && (
          <>
            ...
            {churning.length === 0 ? (
              <Box sx={{ textAlign: 'center', py: 8 }}>
                <Box sx={{ fontSize: 40, mb: 1.5 }}>💚</Box>
                <Typography sx={{ fontWeight: 500, color: c.text }}>הלקוחות שלך נאמנים!</Typography>
                <Typography sx={{ fontSize: 13, color: c.text3 }}>אף לקוח קבוע לא נעלם כרגע</Typography>
              </Box>
            ) : (
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
                {churning.map((ch: ChurningCustomer) => (
                  <Box key={ch.customer.id} className="zk-card" sx={{ bgcolor: c.card, border: `1px solid ${c.border2}`, borderRadius: 4, p: 2.25 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Typography sx={{ fontSize: 15.5, fontWeight: 600, color: c.text }}>{ch.customer.name}</Typography>
                          {ch.visits >= 5 && <Box sx={{ fontSize: 10, fontWeight: 600, color: '#fff', bgcolor: c.amber, borderRadius: 99, px: 0.9, py: 0.15 }}>VIP</Box>}
                        </Box>
                      ...
                      <Box sx={{ display: 'flex', gap: 0.75 }}>
                        <Button href={`tel:${ch.customer.phone}`} size="small" variant="outlined" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12, minWidth: 'auto' }}>📞</Button>
                        <Button onClick={() => openMsg(ch.customer.phone, ch.customer.name, winbackMessage(ch.customer.name))} size="small" variant="contained" sx={{ borderRadius: 4, fontWeight: 500, fontSize: 12, bgcolor: '#25D366', '&:hover': { bgcolor: '#1EA952' } }}>💬 החזר</Button>
                      </Box>
                    </Box>
                  </Box>
                ))}
      ...
      <Dialog open={!!msgDialog} onClose={() => setMsgDialog(null)} fullWidth PaperProps={{ sx: { bgcolor: c.card, borderRadius: 4, maxWidth: 440, m: 2 } }}>
        <Box sx={{ p: 2.75 }}>
          <Typography sx={{ fontSize: 17, fontWeight: 600, color: c.text, mb: 0.25 }}>💬 הודעה ל{msgDialog?.name}</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button href={msgDialog ? waLink(msgDialog.phone, msgText) : '#'} target="_blank" onClick={() => setMsgDialog(null)} fullWidth variant="contained" sx={{ borderRadius: 4, fontWeight: 600, bgcolor: '#25D366', '&:hover': { bgcolor: '#1EA952' } }}>💬 שלח בוואטסאפ</Button>
            <Button onClick={() => { navigator.clipboard?.writeText(msgText); setMsgDialog(null); }} variant="outlined" sx={{ borderRadius: 4, fontWeight: 500, whiteSpace: 'nowrap' }}>📋 העתק</Button>
          </Box>
        </Box>
      </Dialog>
    </Box>
