  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      ...
      <Box sx={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: 1, alignItems: 'center', pointerEvents: 'none', width: 'max-content', maxWidth: '90vw' }}>
        {toasts.map((t) => (
          <Box key={t.id} sx={{ display: 'flex', alignItems: 'center', gap: 1.25, bgcolor: c.surface1, border: `1px solid ${c.border}`, borderRadius: 99, px: 2.5, py: 1.5, boxShadow: c.shadowLg, animation: 'zkToastIn 0.3s cubic-bezier(0.22,1,0.36,1)', '@keyframes zkToastIn': { from: { opacity: 0, transform: 'translateY(12px)' }, to: { opacity: 1, transform: 'translateY(0)' } } }}>
