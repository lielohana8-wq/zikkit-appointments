  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: c.bg }}>
      ...
      <Box sx={{ maxWidth: 680, mx: 'auto', px: { xs: 2.5, sm: 4 }, py: 3 }}>
        <TextField fullWidth placeholder="🔍  חיפוש לפי שם או טלפון" value={search} onChange={(e) => setSearch(e.target.value)} sx={{ mb: 3, '& .MuiOutlinedInput-root': { borderRadius: 99, bgcolor: c.surface1 } }} />
        {customers.length > 0 && (
          <Typography sx={{ fontSize: 13, fontWeight: 700, color: c.text3, mb: 1.5, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{filtered.length} לקוחות</Typography>
        )}
        {filtered.length === 0 ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.25 }}>
            {filtered.map((cu) => (
              <Box key={cu.id} sx={{ bgcolor: c.surface1, border: `1px solid ${c.border}`, borderRadius: 4, p: 2, display: 'flex', alignItems: 'center', gap: 2, boxShadow: c.shadowSm, transition: 'all 0.2s', '&:hover': { boxShadow: c.shadowMd } }}>
                ...
                {cu.phone && <Button href={`tel:${cu.phone}`} size="small" sx={{ minWidth: 40, height: 40, borderRadius: '50%', color: c.accent, bgcolor: c.accentDim, fontSize: 16 }}>📞</Button>}
              </Box>
            ))}
          </Box>
        )}
      ...
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} PaperProps={{ sx: { borderRadius: 5, p: 3.5, maxWidth: 400, width: '100%', maxHeight: '90vh', overflowY: 'auto' } }}>
